...
Sub SaveData(Optional ShowOnMacroList As Boolean = False)
    ...
    With tblRow.Range
        ...
        .Cells(1, colMap("Suplementação")).Value = wsForm.Range("B32").Value
        ...
        If .Cells(1, colMap("Impacto")).Formula = "" Then
            .Cells(1, colMap("Impacto")).Formula = "=IFERROR([@[Custo da Suplementação]]/[@[Custo COT]];"")"
        End If
        If .Cells(1, colMap("Saldo")).Formula = "" Then
            .Cells(1, colMap("Saldo")).Formula = "=[@[Custo Planejado Depois da Suplementação]]-[@[Custo COT]]-[@[Custo da Suplementação]]"
        End If
        
        .Cells(1, colMap("Provisão")).Value = wsForm.Range("B56").Value
        ...
        If wsForm.Range("D14").Value < 0.4 Then
            .Cells(1, colMap("Estágio")).Value = Format(wsForm.Range("D14").Value, "##.00%") & " (Fase Inicial)"
        ElseIf wsForm.Range("D14").Value < 0.8 Then
            .Cells(1, colMap("Estágio")).Value = Format(wsForm.Range("D14").Value, "##.00%") & " (Fase Intermediária)"
        Else
            .Cells(1, colMap("Estágio")).Value = Format(wsForm.Range("D14").Value, "##.00%") & " (Fase Final)"
        End If
...
Sub RetrieveDataFromName(Optional ShowOnMacroList As Boolean = False)
    ...
    For Each cell In dadosTable.ListColumns(colMap("ID")).DataBodyRange
        If cell.Value & " - " & cell.Cells(1, colMap("Cliente")) & " - " & cell.Cells(1, colMap("Obra")) & " - " & cell.Cells(1, colMap("Descrição")) = searchName Then
            Set foundRow = cell
            Exit For
        End If
    Next cell
    ...
    With wsForm
        ...
        .Range("B32").Value = foundRow.Cells(1, colMap("Suplementação")).Value
...
Sub RetrieveDataFromID(Optional ShowOnMacroList As Boolean = False)
    ...
    ' Populate worksheet with retrieved data
    With wsForm
        wsForm.OLEObjects("ComboBoxName").Object.Value = foundRow.Value & " - " & foundRow.Cells(1, colMap("Cliente")) & " - " & foundRow.Cells(1, colMap("Obra")) & " - " & foundRow.Cells(1, colMap("Descrição"))
...
Sub EnviarParaAprovação(Optional ShowOnMacroList As Boolean = False)
    ...
    If foundRow.Cells(1, 29).Value <> "" Then
        userResponse = MsgBox("O e-mail de aprovação para esses dados já foi enviado em " & foundRow.Cells(1, 29).Value & ". Deseja enviar novamente?", vbYesNo)
    ...
    'HTMLbody = HTMLbody & "<p>Solicito sua confirmação (“De acordo”) quanto aos valores abai xo, para que possamos dar continuidade à contratação da " & _
        ...
        " no valor de " & Format(foundRow.Cells(1, 7).Value, "R$ #,##0.00") & ". Todos os valores apresentados abaixo foram analisados pela equipe de Implantação/Suprimentos e considerado procedentes." & "</p>"
    ...
    With OutMail
        ...
        .CC = "matheusp@weg.net"
        .BCC = ""
        .Subject = "Aprovação de Custos - Suplementação de Custos - " & foundRow.Cells(1, 1).Value & " - " & foundRow.Cells(1, 2).Value
...
Public Function GetColumnHeadersMapping() As Object
    ...
    headers.Add "Provisão", 16
    headers.Add "Descrição", 17
    headers.Add "Justificativa", 18
    headers.Add "Estágio", 19
